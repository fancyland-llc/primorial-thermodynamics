...
Matos (2026), DOI: \href{https://doi.org/10.5281/zenodo.19076680}{10.5281/zenodo.19076680}
\end{center}
...
The $\sqrt{p_{\text{max}}}$ enters the scaling law \emph{only} through the Gauss sum, not through eigenvalue spectral algebra---generalizing Theorem~6.2. See \S9 for the $m = 30030$ verification.
...
\medskip
\textbf{Computational verification.} Theorem~9.6 is verified by \texttt{v3\_overnight.py}, which validates the mod-$3^k$ method against known exact valuations at $m = 210$ and $m = 2310$ before computing $m = 30030$. Total runtime ${\sim}\,6$ minutes on a consumer PC.
...
$p_{\text{max}}$-exclusion universal: 52 squarefree $m$ tested, all pass & \textbf{Verified} (Theorem~8.8) & \S8.5 \\
$R(2m) = R(m)$ for odd~$m$ (factor of 2 irrelevant) & \textbf{Verified} (15 pairs; Theorem~8.9) & \S8.5 \\
...
$p_{\text{max}}$-exclusion holds at all 5 primorial levels ($k = 2, \ldots, 6$) & \textbf{Verified} (Conjecture~8.1; universal) & \S8.5/\S9 \\
Conjecture~8.2 (ratio always integer) \textbf{FALSIFIED} at $m = 30030$ & \textbf{Proved} (counterexample) & \S9 \\
...
Hierarchy breaks: $b_4 = -1062916/3$ (not integer; Theorem~9.4) & \textbf{Proved} & \S9 \\
88 independent assertions, 0 failures (\texttt{hierarchy\_termination\_proof.py}) & \textbf{Verified} & \S9 \\
$v_3(\det_{\text{odd}}(30030)) = 947$ (prime); $v_3(\det_{\text{even}}(30030)) = 946 = 2 \times 11 \times 43$ & \textbf{Computed} (mod-$3^k$, converged at $k{=}3$; Theorem~9.6) & \S9.6 \\
...
\texttt{v3\_overnight.py}              & --- & Exact $v_3(\det_e)$, $v_3(\det_o)$ via mod-$3^k$ Gaussian elimination ($k = 1, 2, 3$); Theorem~9.6 \\
